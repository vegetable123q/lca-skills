...
| 2 | `built_model_count: N` for N processes | `lifecyclemodel auto-build` grouped separate run dirs into separate single-node models. | Use the driver so all ILCD datasets land in one source-specific combined run dir. |
...
| 7 | Missing material quantities are filled with fake kg values | Critical material, product, or operation data are missing after measured, calculated, and estimated derivations. | Keep quantified steps non-black-box; mark only the affected unit operation `black_box: true`, use `unit:item`, amount `1`, and explain the critical data gap. |
| 8 | Source patent or run outputs are not tracked by git | `data/` and `output/` are gitignored. | Archive the run folder or publish with `--publish-only --commit` after review. |
...
| 11 | `missing .../orchestrator-run/publish-bundle.json` during publish | Stage 6 publish prep has not produced the standard bundle yet. | Run `--all` first, then retry `--publish-only --commit`. |
