...
- 专利有明确数值时优先用专利；没有数值时，只能使用可审计的计算、工程默认值或黑箱 item fallback。
...
如果一个专利步骤存在，但物料数量完全不可辩护，不要伪造 kg 级清单。应设置：

- process: `"black_box": true`
- 该 process 使用的所有 flow: `"unit": "item"`
- 该 process 的每个 exchange: `"amount": 1`
- `comment` 中说明“源文本没有可辩护的数量，因此使用 item-based black-box”
...
4. 直接从 plan 写出 `runs/combined/exports/processes/<PROC_UUID>_00.00.001.json`。
5. 从第一个 scaffold run 复制 `cache/process_from_flow_state.json` 和 `manifests/*.json` 到 `runs/combined/`，满足 lifecyclemodel builder 的本地 run 形状要求。
6. 写 `manifests/lifecyclemodel-manifest.json`，指向单一 combined run。
...
builder 读取 `runs/combined/exports/processes/`。它通过 flow UUID 推断边：
...
- `lifecyclemodel-run/models/combined/tidas_bundle/lifecyclemodels/<MODEL_UUID>_<version>.json`
- `lifecyclemodel-run/models/combined/summary.json`
- `lifecyclemodel-run/models/combined/connections.json`
...
| `built_model_count: N` | 多个 process 被放在多个 run 里。 | 使用 driver 的 single combined run。 |
...
1. 估算器应输出不确定性范围。目前大多返回单点值，适合可复现但不适合表达专利缺数带来的不确定性。
2. EIA 与参考文献来源仍以脚本文本常量为主。后续应迁移到版本化 `references/` 数据表，避免公式、来源和代码耦合。
3. `plan.json` 还没有独立 JSON Schema。当前校验在 `normalize-plan.mjs` 中，适合运行时防错，但不利于编辑器提示和批量 lint。
4. 复杂共产品、回流和分配规则表达不足。现在主要依赖共享 `flow_key` 推边，缺少显式 allocation 或 coproduct 语义。
5. 黑箱 fallback 过粗。`item` 可以防止伪造质量清单，但会牺牲后续 LCA 可解释性；应增加 data quality、uncertainty 或 review status 字段。
6. 重跑清理需要手动执行。driver 可以增加受控 `--clean-generated`，只删除生成工件，保留 authored plan。
7. `materialize-from-plan.mjs` 生成 ILCD 时带当前时间戳，严格复现实验会受影响。若需要 byte-for-byte reproducible outputs，应支持固定 timestamp。
8. 估算器的 waste 模式目前偏 NCM/CAM。其他行业专利需要独立参数集，不应复用锂电材料 EIA 因子。
9. 示例 plan 与模板还可以更短。详细审计字段应在说明文档中解释，模板保留最小字段和一个审计字段示例即可。
